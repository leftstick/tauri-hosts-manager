...
import { addNewOSHost, deleteOSHosts, getOSHosts, modifyOSHost } from '@/os';
...
export async function changeHostField(
  ...
  if (fieldName === 'disabled') {
    if (value === true) {
      ...
      // del from sys
      await deleteOSHosts(originalHost);
      return getAllHosts();
    }
    if (value === false) {
      const disabledHosts = getDisabledHosts();
  ...
  if (fieldName === 'domain' && !originalHost.disabled) {
    await modifyOSHost(fieldName, value as string, originalHost);
    return getAllHosts();
  }
